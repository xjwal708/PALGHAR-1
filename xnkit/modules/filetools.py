...
async def unzipfiles(zippath):
    ...
    extract_path = f"asterix/downloads/{foldername}"
...
async def zip_handler(_, m: Message):
    ...
    if not reply:
        return await app.send_edit(
            m, f"Reply to some media file . . .", text_type=["mono"], delme=4
        )

    elif reply:
        if not reply.media:
            return await app.send_edit(
                m, "Reply to some media not text . . .", text_type=["mono"]
            )
        ...
        if app.TEMP_DICT:
            ...
            pass
        dl = await app.download_media(reply, block=False)
        zipfile.ZipFile(dl.replace("/app/downloads/", "") + ".zip", "w").write(dl)
        place = dl.replace("/app/downloads/", "") + ".zip"
        await app.send_edit(
            m, f"**Your file is compressed and saved here:** \n`{place}`"
        )
    else:
        await app.send_edit(
            m, "Something went wrong . . .", delme=2, text_type=["mono"]
        )
...
async def createfile_handler(app, m: Message):
    ...
    try:
        ...
        if reply and app.long(m) >= 2:
            ...
            await app.create_file(message=m, filename=name, content=text, send=True)
        ...
        # if replied to text without file name
            ...
            await app.create_file(
                message=m, filename="file.py", content=text, send=True
            )
