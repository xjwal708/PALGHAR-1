...
from pyrogram.errors import PeerIdInvalid, UserAdminInvalid, UsernameNotOccupied
...
async def admintitle_handler(_, m: Message):
    ...
    if await app.IsAdmin(m) is True:
        if app.long(m) == 3:
            try:
                ...
                try:
                    ...
                    return await app.send_edit(
                        m,
                        "The username | id seems to be invalid  . . .",
                        text_type=["mono"],
                        delme=4,
                    )
                await app.send_edit(
                    m, f"Setting `{user_data}`s title as `{title}` . . ."
                )
                admin = user.id
                ...
                if is_admin == "member":
                    await app.send_edit(
                        m,
                        f"{user_name} is not an admin in this chat, use {app.PREFIX}promote command to promote them.",
                        delme=3,
                        text_type=["mono"],
                    )
                else:
            ...
            await app.send_edit(
                m,
                "Reply or give me username | id after command . . .",
                text_type=["mono"],
                delme=3,
            )
        else:
            await app.send_edit(
                m, "Something went wrong, Try again later.", text_type=["mono"], delme=3
            )
    else:
        await app.send_edit(
            m, "Sorry you are not an admin here . . .", delme=3, text_type=["mono"]
        )
...
async def invite_handler(_, m):
    ...
    if reply:
        ...
        return await app.send_edit(
            m, "I can't invite ghost, can I ?", text_type=["mono"]
        )
...
async def adminlist_handler(_, m):
    ...
    async for x in app.iter_chat_members(m.chat.id, filter="administrators"):
        if x.status == "creator":
            creator.append(
                "{}".format(app.MentionMarkdown(x.user.id, x.user.first_name))
            )
    ...
    if reply:
        if text:
            teks = "{} is reported to admins.\n**Reason:** {}".format(
                app.MentionMarkdown(reply.from_user.id, reply.from_user.first_name),
                text,
            )
        else:
            teks = "{} is reported to admins.".format(
                app.MentionMarkdown(reply.from_user.id, reply.from_user.first_name)
            )
    else:
        return await app.send_edit(
            m,
            "Reply to someone to report him to admins . . .",
            delme=4,
            text_type=["mono"],
        )
    teks += " ".join(admin)
...
async def tagall_handler(app, m: Message):
    ...
    if reply:
        await app.send_message(
            m.chat.id, text, reply_to_message_id=reply.message_id, parse_mode="html"
        )
    else:
...
async def membercount_handler(_, m):
    ...
    if app.long(m) == 1:
        try:
            ...
            await app.send_edit(
                m, "The username | id does not exist . . .", text_type=["mono"]
            )
        ...
        try:
            ...
            await app.send_edit(
                m, "The username does not exist . . .", text_type=["mono"]
            )
    else:
        await app.send_edit(
            m,
            f"Usage: `{app.PREFIX}members` or `{app.PREFIX}members [chat username | id]` ",
            delme=5,
        )
...
async def joinchat_handler(_, m: Message):
    if app.long(m) == 1:
        await app.send_edit(
            m,
            "Give me some chat id | username after command . . .",
            text_type=["mono"],
            delme=5,
        )
    elif app.long(m) > 1:
        ...
        await app.send_edit(
            m, "Maximum 4096 characters allowed . . .", text_type=["mono"], delme=5
        )
    ...
    if await app.IsAdmin(m) is True:
        if app.long(m) == 1:
            ...
            if sec == "off":
                ...
                await app.send_edit(
                    m, "Slow mode is now turned off.", delme=3, text_type=["mono"]
                )
            elif int(sec) not in [10, 30, 60, 300, 900, 3600]:
                return await app.send_edit(
                    m,
                    "Please choose seconds from here: [`10`, `30`, `60`, `300`, `900`, `3600`]",
                )
        elif app.long(m) > 4096:
            return await app.send_edit(
                m, "Only 4096 characters are allowed . . .", text_type=["mono"], delme=3
            )
        ...
        await app.send_edit(
            m, "Sorry, you are not an admin here . . .", delme=4, text_type=["mono"]
        )
