...
async def ls_handler(_, m: Message):
    ...
    for file in files:
        if not file.endswith(".session") and not file in [
            "__pycache__",
            ".git",
            ".github",
            ".profile.d",
            ...
            if os.path.isfile(f"{location}/{file}"):
                collect.append(
                    f"📑 `{file}` ({app.DictSize(os.path.abspath(location+file))})"
                )
            if os.path.isdir(f"{location}/{file}"):
                collect.append(
                    f"🗂️ `{file}` ({app.DictSize(os.path.abspath(location+file))})"
                )
    ...
    if len(OUTPUT) > 4096:
        await m.delete()
        await app.create_file(m, app, filename="dict.txt", text=OUTPUT)
...
async def download_handler(_, m: Message):
    ...
    if reply and reply.media:
        try:
            ...
            if location is None:
                await app.send_edit(
                    m, "Download failed, please try again.", text_type=["mono"]
                )
            else:
                await app.send_edit(
                    m,
            ...
            await app.error(m, e)
            await app.send_edit(
                m, f"Failed To Download, look in log chat for more info."
            )
        ...
        try:
            ...
            while not downloader.isFinished():
                ...
                try:
                    ...
                    current_message += f"__{app.HumanBytes(downloaded)} of {app.HumanBytes(total_length)}__\n"
                    current_message += f"**Speed:** __{speed}__\n"
                    current_message += f"**ETA:** __{estimated_total_time}__"
                    if round(diff % 10.00) == 0 and current_message != display_message:
                        await app.send_edit(
                            m, disable_web_page_preview=True, text=current_message
                        )
            ...
            return await app.send_edit(m, f"Failed Download!\n\n{exc}")
    else:
        await app.send_edit(
            m,
...
async def upload_handler(_, m: Message):
    if app.long(m) > 1:
        ...
        if os.path.exists(local_file_name):
            ...
            await app.send_edit(
                m, "404: directory not found . . .", text_type=["mono"], delme=5
            )
    else:
        await app.send_edit(
            m,
            f"`{app.PREFIX}upload [file path ]` to upload to current Telegram chat",
            delme=4,
        )
        ...
        try:
            ...
            for file in files:
                if file.endswith(".py"):
                    ...
                    await app.send_edit(
                        m, f"Uploaded all files from Directory `{temp_dir}`", delme=3
                    )
