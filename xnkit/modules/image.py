...
async def get_image(m: Message, keyword):
    await app.send_edit(m, "Getting Picture . . .", text_type=["mono"])
    img = await app.get_url(f"https://source.unsplash.com/1600x900/?{keyword}")
    await asyncio.gather(m.delete(), app.send_photo(m.chat.id, str(img)))
...
async def unsplash_handler(_, m: Message):
    ...
    if app.long(m) == 1:
        await app.send_edit(
            m, "Give me some query after command . . .", text_type=["mono"], delme=4
        )
    elif app.long(m) == 2:
        if cmd[1].isdigit():
            return await app.send_edit(
                m, "Sorry but give me a text query.", text_type=["mono"], delme=4
            )
        else:
            keyword = cmd[1]
        ...
        if cmd[1].isdigit():
            if app.is_str(cmd[2]):
                ...
                for img in images:
                    await asyncio.gather(app.send_photo(m.chat.id, str(img)))
                if m.from_user.is_self:
                    await m.delete()
            else:
                await app.send_edit(
                    m,
                    "Sorry numbers are not allowed to be a search query.",
                    delme=4,
                    text_type=["mono"],
                )
        else:
    ...
    if not reply:
        ...
        if reply.sticker:
            if not reply.sticker.is_animated:
                ...
                await app.download_media(message=reply, file_name=filename)
                await app.send_photo(
                    m.chat.id, filename, reply_to_message_id=reply.message_id
                )
                await m.delete()
            ...
            await app.send_edit(
                m, "Reply to a sticker please !", delme=2, text_type=["mono"]
            )
        ...
        if reply.photo or reply.document.file_name.endswith(".png" or ".jpg" or "jpeg"):
            if not reply.video:
                m = await app.send_edit(
                    m, "Converting To Sticker . . .", text_type=["mono"]
                )
            ...
            await app.send_edit(
                m, "Reply to supported media . . .", delme=3, text_type=["mono"]
            )
...
async def qrcode_handler(_, m):
    try:
        ...
        await app.send_document(m.chat.id, picname)
...
async def colourtemplate_handler(_, m: Message):
    if app.long(m) == 1:
        await app.send_edit(
            m, "Please give some colour name after command . . .", delme=3
        )
    ...
    if reply:
        ...
        if app.long(m) > 1:
            try:
                ...
                await app.send_edit(
                    m, "Sorry this username does not exist . . .", text_type=["mono"]
                )
