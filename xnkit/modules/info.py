...
async def mediainfo_handler(_, m: Message):
    ...
    if not replied:
        return await app.send_edit(
            m, "Please reply to some media to get media info . . .", text_type=["mono"]
        )
    ...
    try:
        ...
        if poto and data:
            await app.send_cached_media(
                m.chat.id, file_id=poto[0].file_id, caption=data
            )
            await m.delete()
        elif not poto:
            ...
            await app.send_edit(
                m, "Failed to get information of this group . . .", delme=2
            )
