    ...
    try:
        ...
        if file_id and not file_id.startswith("#"):  # as a text
            return await app.send_message(
                m.chat.id, f"{file_id}", reply_to_message_id=m.message_id
            )
        ...
        if caption:
            ...
            await app.send_cached_media(
                chat_id=m.chat.id, file_id=file_id, reply_to_message_id=m.message_id
            )
...
async def savewelcome_handler(_, m: Message):
    ...
    await app.send_edit(
        m, "Setting this media as a welcome message . . .", text_type=["mono"]
    )
    ...
    if reply:
        try:
            if bool(reply.media) is True:
                ...
                await app.send_edit(
                    m,
                    "Added this media to welcome message . . .",
                    delme=2,
                    text_type=["mono"],
                )
                ...
                await app.send_edit(
                    m,
                    "Added this text to welcome message . . .",
                    delme=2,
                    text_type=["mono"],
                )
        except Exception as e:
...
async def deletewelcome_handler(_, m: Message):
    ...
    try:
        await app.send_edit(
            m, "Checking welcome message for this group . . .", text_type=["mono"]
        )
        app.del_welcome(str(m.chat.id))
        await app.send_edit(
            m,
            "Successfully deleted welcome message for this chat . . .",
            delme=2,
            text_type=["mono"],
        )
    except Exception as e:
...
async def getwelcome_handler(_, m: Message):
    ...
    try:
        ...
        if text is None and cap is None:
            await app.send_edit(
                m,
                "No welcome message was assigned to this group.",
                text_type=["mono"],
                delme=3,
            )
