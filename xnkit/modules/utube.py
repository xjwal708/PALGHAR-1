...
async def videoinfo_handler(_, m: Message):
    ...
    if reply and reply.text:
        ...
        return await app.send_edit(
            m,
            "Reply to youtube link or give link as a suffix . . .",
            text_type=["mono"],
            delme=5,
        )
...
async def ytdownload_handler(_, m):
    ...
    if not reply:
        if app.long(m) == 1:
            return await app.send_edit(
                m,
                "Please reply to a yt link or give me link as a suffix . . .",
                text_type=["mono"],
                delme=4,
            )
        elif (
            app.long(m) > 1
            and m.command[1].startswith("http://" or "https://")
            and not m.command[1].isdigit()
        ):
            link = m.command[1]
        else:
            return await app.send_edit(
                m,
                "Please reply to a link or give me the link as a suffix after command . . .",
                text_type=["mono"],
                delme=4,
            )
    elif reply:
    ...
    for x in data:
        if (
            x.type == "video"
            and x.resolution in ("720p" or "1080p")
            and x.mime_type == "video/mp4"
        ):
            try:
                loc = x.download()
