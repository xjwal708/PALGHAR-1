...
def get_soup(url, header):
    return BeautifulSoup(
        urllib.request.urlopen(urllib.request(url, headers=header)), "html.parser"
    )
    ...
    try:
        ...
        if not reply:
            return await app.send_edit(
                m, "Reply to some media.", text_type=["mono"], delme=4
            )

        if reply.photo:
            m = await app.send_edit(m, "⏳ • Hold on ...")
            savename = "photo_{}_{}.png".format(reply.photo.file_id, reply.photo.date)
            await app.download_media(reply, file_name="./downloads/" + savename)
        elif reply.animation:
            m = await app.send_edit(m, "⏳ • Hold on ...")
            savename = "giphy_{}-{}.gif".format(
                reply.animation.date, reply.animation.file_size
            )
            await app.download_media(reply, file_name="./downloads/" + savename)
        else:
            return await app.send_edit(
        ...
        await app.send_edit(
            m, "Results: [Tap Here]({})".format(fetchUrl), disable_web_page_preview=True
        )
...
async def yandeximages_handler(_, m: Message):
    ...
    try:
        if app.long(m) > 1:
            ...
            result = await app.get_inline_bot_results("@pic", photo)
            await m.delete()
...
async def imagesearch_handler(_, m: Message):
    ...
    if app.long(m) == 1:
        return await app.send_edit(
            m, "Please give me some query.", text_type=["mono"], delme=4
        )
