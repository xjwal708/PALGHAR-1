...
async def savenote_hanlder(_, m: Message):
    ...
    if not note_name:
        return await app.send_edit(
            m, "A note name is necessary to save a note !", text_type=["mono"]
        )
...
async def getnote_handler(_, m: Message):
    ...
    if m.text and m.text.startswith(">"):
        ...
        if getnotes["type"] == app.TEXT:
            ...
            if button:
                return await app.send_edit(
                    m, "Inline button not supported in this userbot version :("
                )
            else:
            ...
            app.VIDEO_NOTE,
            app.CONTACT,
            app.ANIMATED_STICKER,
        ):
            await m.delete()
            try:
                if msg_id:
                    await GET_FORMAT[getnotes["type"]](
                        message.chat.id, getnotes["file"], reply_to_message_id=msg_id
                    )
                else:
                    await GET_FORMAT[getnotes["type"]](
                ...
                if msg_id:
                    await GET_FORMAT[getnotes["type"]](
                        m.chat.id, content, reply_to_message_id=msg_id
                    )
                else:
            ...
            if button:
                return await app.send_edit(
                    m, "Inline button not supported in this userbot."
                )
            else:
                try:
                    if msg_id:
...
async def notelist_handler(_, m: Message):
    ...
    if not getnotes:
        return await app.send_edit(
            m, "There are no saved notes !", text_type=["mono"], delme=4
        )
...
async def clearnote_handler(_, m: Message):
    if app.long(m) <= 1:
        return await app.send_edit(
            m, f"Sir, give me a note name after command, Ex: `{app.PREFIX}clear cat`"
        )
