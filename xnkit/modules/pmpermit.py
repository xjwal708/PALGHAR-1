...
async def old_msg(m: Message, user_id):
    ...
    if bool(app.get_msgid(user_id)) is True:
        ...
        await app.delete_messages(chat_id=m.chat.id, message_ids=old_msgs)
        return True
    return False
...
async def send_warn(m: Message, user):
    ...
    if pic:
        msg = await app.send_photo(m.chat.id, pic, caption=text)
        app.set_msgid(user, msg.message_id)
        return True
    elif not pic:
        msg = await app.send_message(m.chat.id, text, disable_web_page_preview=True)
        app.set_msgid(user, msg.message_id)
        return True
    else:
...
async def pmpermit_handler(_, m: Message):
    try:
        ...
        if (
            bool(app.Pmpermit()) is False or m.chat.is_verified
        ):  # allow verified accounts
            return
            ...
            if warn < pmlimit:
                ...
                if done:
                    try:
                        ...
                        print(
                            f"{user.first_name} was blocked in your pm for some reason."
                        )
                else:
                    await app.send_edit(
                        m,
...
async def diapprove_handler(_, m: Message):
    ...
    if m.chat.type == "private":
        ...
        if reply:
            ...
            return await app.send_edit(
                m, "Whom should i disapprove, piro ?", text_type=["mono"], delme=4
            )
            ...
            try:
                ...
                return await app.send_edit(
                    m, "Pass username instead of user id.", text_type=["mono"], delme=4
                )
            except UsernameNotOccupied:
                return await app.send_edit(
                    m,
            ...
            return await app.send_edit(
                m, "Failed to disapprove user !", text_type=["mono"], delme=4
            )
    if user_data:
        info = user_data
    else:
    ...
    if info:
        ...
        await app.send_edit(
            m, f"{info.mention} `has been disapproved for pm.`", delme=4
        )
        try:
            await app.send_message(
                app.LOG_CHAT, f"#disallow\n\n{info.mention} `has been disapproved.`"
            )
        except PeerIdInvalid:
            print(f"{info.first_name} has been disapproved.")
    else:
        return await app.send_edit(
            m, "Sorry there is no user id to disapprove.", text_type=["mono"], delme=4
        )
