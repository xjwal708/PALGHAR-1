...
async def addsudo_handler(_, m: Message):
    ...
    await app.send_edit(
        m, f"{reply.from_user.mention()} `has been added to sudo.`", delme=4
    )
...
async def delsudo_handler(_, m: Message):
    ...
    if user_id in sudo_list:
        ...
        return await app.send_edit(
            m, "This user is not in sudo list", text_type=["mono"], delme=4
        )

    await app.send_edit(
        m, f"{reply.from_user.mention()} `has been removed from sudo list`", delme=4
    )
