...
async def herokuvars_handler(_, m: Message):
    ...
    try:
        m = await app.send_edit(
            m, "Fetching all vars from Heroku . . .", text_type=["mono"]
        )
...
async def setvar_handler(_, m: Message):
    ...
    if app.long(m) < 3:
        await app.send_edit(m, f"`{app.PREFIX}setvar [key] [value]`")
    elif app.long(m) >= 3:
        key = m.command[1]
        value = m.command[2]
        heroku_vars = heroku_app.config()
        try:
            if key and value in heroku_vars:
                await app.send_edit(m, f"{key} is already in vars with value {value}")
...
async def getvar_handler(_, m: Message):
    ...
    if app.long(m) == 1:
        return await app.send_edit(m, f"`{app.PREFIX}getvar [key name]`")
    elif app.long(m) >= 2:
        key = m.command[1]
        heroku_vars = heroku_app.config()
        try:
            if heroku_vars:
                ...
                await app.send_edit(
                    m, "Failed to get heroku key . . .", text_type=["mono"]
                )
        except Exception as e:
            await app.error(m, e)
...
async def delvar_handler(_, m: Message):
    ...
    if app.long(m) == 1:
        return await app.send_edit(
            m, f"{app.PREFIX}delvar [key name]", text_type=["mono"]
        )
    elif app.long(m) >= 2:
        m = await app.send_edit(
            m, "Verifying var in heroku config vars . . .", delme=3, text_type=["mono"]
        )
        ...
        if key not in heroku_vars:
            return await app.send_edit(
                m, f"**`{key}`** does not exist in heroku vars . . .", delme=3
            )
        ...
        await app.send_edit(
            m, f"Successfully deleted var = [ {key} ] from heroku vars !", delme=4
        )
    else:
        await app.send_edit(
            m, f"Usage: `{app.PREFIX}delvar [key name]` use this format.", delme=4
        )
