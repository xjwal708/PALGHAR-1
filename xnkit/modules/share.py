...
async def sendmodule_handler(app, m: Message):
    if app.long(m) > 1:
        ...
        modulename = f"xnkit/modules/{filename}.py"
        if os.path.exists(modulename):
            ...
            await app.send_edit(
                m, "404: plugin not found . . .", delme=2, text_type=["mono"]
            )
    else:
        await app.send_edit(
            m, f"`{app.PREFIX}send [ plugin name ]`  to upload plugin file.", delme=3
        )
...
async def install_handler(_, m: Message):
    ...
    if not reply:
        return await app.send_edit(
            m, "Reply to a python file to install . . .", text_type=["mono"], delme=4
        )
    if reply:
        if not reply.document.file_name.endswith(".py"):
            return await app.send_edit(
                m, "Only (.py) modules can be installed !!", text_type=["mono"], delme=2
            )
        doc_name = reply.document.file_name

        module_loc = f"asterix/modules/{doc_name}"
        ...
        try:
            download_loc = await app.download_media(message=reply, file_name=module_loc)
            if download_loc:
    ...
    try:
        if app.long(m) > 1:
            if cmd[1].endswith(".py"):
                module_loc = f"xnkit/modules/{cmd[1]}"
            elif not cmd[1].endswith(".py"):
                module_loc = f"xnkit/modules/{cmd[1]}.py"
            if os.path.exists(module_loc):
                os.remove(module_loc)
                await app.send_edit(m, f"**Uninstalled module:** {cmd[1]}", delme=5)
            else:
                await app.send_edit(
                    m, "Module doesn't exist !", delme=4, text_type=["mono"]
                )
        else:
            await app.send_edit(
                m, "Give me a module name . . .", text_type=["mono"], delme=4
            )
    except Exception as e:
        await app.error(m, e)
