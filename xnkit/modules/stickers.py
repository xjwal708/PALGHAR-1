...
async def kang(_, m: Message):
        ...
        if replied.photo:
            ...
            if not replied.sticker.file_name:
                return await app.send_edit(
                    m, "Sticker has no Name !", text_type=["mono"]
                )
    ...
    if photo:
        ...
        custom_packnick = f"{app.username}'s <\> @XnKiTKuMaR <\>"
        ...
        if exist is not False:
            ...
            while limit in await get_response(m):
                ...
                if is_anim:
                    ...
                    packnick += " (Animated)"
                await app.send_edit(
                    m, "Switching to Pack " + str(pack) + " due to insufficient space"
                )
                ...
                if await get_response(m) == "Invalid pack selected":
                    ...
                    await app.send_edit(
                        m, f"**Sticker** {out} __in a Different Pack__**!**"
                    )
                    return
            await app.send_document("Stickers", photo)
            time.sleep(0.2)
