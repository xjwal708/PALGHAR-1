    ...
    if reply:
        ...
        if str(reply.from_user.id) in collect:
            if prefix[0] == "+":
                ...
                collect.update(
                    {str(reply.from_user.id): str(int(data) + int(prefix[1]))}
                )
                await app.bot.send_message(
                    m.chat.id,
                    f"{reply.from_user.first_name}: "
                    + str(int(data) + int(prefix[1]))
                    + " increments",
                )
            elif prefix[0] == "-":
                data = collect.get(str(reply.from_user.id))
            ...
            if prefix[0] == "+":
                ...
                collect.update(data)
                await app.bot.send_message(
                    m.chat.id, f"{reply.from_user.first_name}: 1 increments"
                )
            elif prefix[0] == "-":
                data = {str(reply.from_user.id): str(-1)}
                collect.update(data)
